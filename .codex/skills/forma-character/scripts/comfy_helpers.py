...
def load_config(config_path: Path) -> dict[str, Any]:
    if not config_path.exists():
        raise GenerationError(
            ...
            next_action="Run prepare-environment first or create config/forma-character.json.",
        )
    ...
    if not isinstance(comfyui_url, str) or not comfyui_url.strip():
        raise GenerationError(
            ...
            next_action="Add 'comfyui_url' to config/forma-character.json.",
        )
    if not isinstance(output_dir, str) or not output_dir.strip():
        raise GenerationError(
            detail="Config is missing a valid 'output_dir' string.",
            next_action="Add 'output_dir' to config/forma-character.json.",
        )
    return payload
...
def _candidate_project_name(value: str) -> str:
    head = value.strip().splitlines()[0]
    for separator in (",", "。", ". "):
...
def derive_project_name(*candidates: str | None, fallback: str = "project") -> str:
    for candidate in candidates:
        if not candidate:
            continue
        slug = sanitize_slug(_candidate_project_name(candidate), fallback="")
        if slug:
            return slug[:80]
    return fallback


def infer_project_name_from_output_path(path: Path, output_root: Path) -> str | None:
    try:
        relative = path.resolve().relative_to(output_root.resolve())
    except ValueError:
        return None

    if len(relative.parts) >= 2 and relative.parts[1] == "images":
        slug = sanitize_slug(relative.parts[0], fallback="")
        return slug or None
    return None


def prepare_project_output_paths(
    output_root: Path,
    project_name: str,
    group_name: str,
    output_name: str,
    ...
    run_id = f"{timestamp}-{sanitize_slug(output_name, fallback=group_name)}"
    run_dir = output_root / project_name / "images" / group_name / run_id
    images_dir = run_dir / "images"
    logs_dir = output_root / project_name / "logs"
    try:

...
def main() -> int:
    ...
    try:
        ...
        reference_image_path = resolve_repo_path(args.reference_image, repo_root) if args.reference_image else None
        workflow_payload = load_workflow_payload(workflow_path)

        seed = args.seed if args.seed is not None else secrets.randbelow(2**53)
        uploaded_reference = upload_reference_image(comfyui_url, reference_image_path) if reference_image_path else None

        timestamp = datetime.now(UTC).strftime("%Y%m%d-%H%M%S")
        run_id, character_dir, logs_dir = prepare_output_paths(output_root, args.output_name, timestamp)
        placeholders = build_placeholders(args.character_prompt, seed, run_id, uploaded_reference)
        ...
        history_entry = wait_for_history(comfyui_url, prompt_id, args.timeout_seconds, args.poll_interval)
        ...
        save_json(
            metadata_path,
            {
                "command": "generate-character-sheet",
                "run_id": run_id,
        ...
        print("generate-character-sheet")
        print(f"Run id: {run_id}")
        print(f"Prompt id: {prompt_id}")
        print(f"Character output: {character_dir}")

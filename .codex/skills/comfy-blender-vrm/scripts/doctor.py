...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate the local ComfyUI and Blender setup for the comfy-blender-vrm skill."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def check_comfyui(url: str) -> CheckResult:
    ...
    return CheckResult(
        ...
        ok=False,
        detail=detail,
        next_action="Start ComfyUI and confirm the configured URL is correct, for example http://127.0.0.1:8188.",
    )
...
def main() -> int:
    ...
    print("comfy-blender-vrm doctor")
